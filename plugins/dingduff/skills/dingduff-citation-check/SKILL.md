...
description: Verify every citation in a drafted legal memo (Markdown, Word, or Google Docs) against stored opinions and statutes plus any other source documents the attorney supplies (a Restatement section, an off-CourtListener case, an opposing brief, a factual PDF). Use after drafting a memo when the user asks to cite-check, citecheck, verify citations, verify quotes, check cites, or validate authorities. Produces cites.json, opens the interactive attorney review panel, and records the attorney's verdicts in review.json. (v1.9)
---
...
Privacy: verification runs entirely in this session — the memo and any
supplied documents are read locally by the verifier script and never sent
to DingDuff. When the review panel opens via `citecheck_review`, the memo
and any document sources transit the DingDuff server once as tool
arguments and are never stored or logged. State this plainly if the user
asks.
...
This renders the two-pane review panel in the conversation. Check the
tool result's `structuredContent.panel` field: if it is `"may_not_render"`,
the connected client doesn't support MCP Apps — tell the user to open
the memo in Claude.ai (web, Desktop, or mobile) or Cowork, which support
the panel, rather than waiting for one that won't appear in the current
client.
...
Explain next steps: review each citation in the panel (← / → to move
between citations, 1–3 to record a verdict), then click "Export
review.json" — the panel hands the verdicts back to this conversation
when the attorney is done. The panel also has a "Generate Printable
Review Audit Log" button that downloads a self-contained record of the
review for the matter file.
...
## Step 9 — Printable audit log

The audit record is generated by the panel itself — the attorney clicks
"Generate Printable Review Audit Log" in the panel header, which opens
the print dialog (save to PDF) or downloads a self-contained
`cite-check-audit.html` if the embedded view blocks printing. The audit
records, per citation: the source, the proposition, whether the verifier
anchored it, the attorney's verdict (✅ ⚠️ ‼️ ❓), the review note, and the
reviewer — plus an integrity footer (memo hash, verification provenance).
That is the firm's record that the work product was checked. The
attorney can generate the audit either after recording verdicts or
before any review (all rows show ❓ in the latter case).
